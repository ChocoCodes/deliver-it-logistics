...
public class Driver extends Employee {
    ...
    private final int SHIP_STATUS_COL = 8 - ARRAY_OFFSET;
    private final int SHIP_VH_ID_COL = 3 - ARRAY_OFFSET;
    private final int VEHICLE_CURR_SHIP_COL = 9 - ARRAY_OFFSET;
    private final int VEHICLE_CURR_CAP_COL = 7 - ARRAY_OFFSET;
    private final int VEHICLE_DRIVER_COL = 5 - ARRAY_OFFSET;
    ...
    public void assignVehicle() {
        ...
        CSVParser.setFilePath("CSVFiles/vehicles.csv");
        CSVParser.updateCSV(currentAssignedVehicle.getVehicleID(), currentAssignedVehicle.getDriver(), VEHICLE_DRIVER_COL, currentAssignedVehicle.getVehicleHeader()); // Assigned Vehicle

        // Check if there was a previous assigned vehicle and update its record in the CSV
        if (previousAssignedVehicle != null) {
            CSVParser.updateCSV(previousAssignedVehicle.getVehicleID(), previousAssignedVehicle.getDriver(), VEHICLE_DRIVER_COL, previousAssignedVehicle.getVehicleHeader()); // Remove the previous driver's name
        }
    }
    ...
    public void deliverPackage() {
        ...
        if (!hasPendingShipments) {
            ...
            if(toDeliver.equalsIgnoreCase("Yes")) {
                ...
                CSVParser.setFilePath("CSVFiles/shipments.csv");
                CSVParser.updateCSV(selectedShipment.getShipmentID(), selectedShipment.getStatus(), SHIP_STATUS_COL, selectedShipment.getShipmentHeader());
                CSVParser.updateCSV(selectedShipment.getShipmentID(), String.valueOf(selectedShipment.getVehicleId()), SHIP_VH_ID_COL, selectedShipment.getShipmentHeader());
                    
                // Save Updates to Vehicle CSV
                CSVParser.setFilePath("CSVFiles/vehicles.csv");
                CSVParser.updateCSV(currentAssignedVehicle.getVehicleID(), String.valueOf(currentAssignedVehicle.getCurrentShipmentCount()), VEHICLE_CURR_SHIP_COL, currentAssignedVehicle.getVehicleHeader());
                CSVParser.updateCSV(currentAssignedVehicle.getVehicleID(), String.valueOf(currentAssignedVehicle.getCurrentCapacityKG()), VEHICLE_CURR_CAP_COL, currentAssignedVehicle.getVehicleHeader());
